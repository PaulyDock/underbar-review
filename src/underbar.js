(function() {
  ...
  _.uniq = function(array) {
    ...
    _.each(array, (element) => {
      dupeFreeObj[element] = element;
    });
    for (let key in dupeFreeObj) {
      dupeFreeArray.push(dupeFreeObj[key]);
    }
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    _.each(collection, (element, indexOrKey, iteratedObj) => {
      if (initializing) {
        initializing = false;
        accumulator = element;
      } else {
        accumulator = iterator(accumulator, element, indexOrKey, iteratedObj);
      }
    });
    return accumulator; 
  };
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, (allPassed, element) => {
      return !!iterator(element) && allPassed;
    }, true);
  };
  ...
  _.extend = function(obj) {
    for (let i = 1; i < arguments.length; i++) {
      ...
      _.each(argObj, (value, key, argObj) => {
        obj[key] = value;
      });
    }
    return obj;
  ...
  _.shuffle = function(array) {
    let indexCache = [];
    let randomizedArray = [];
    while (indexCache.length < array.length) {
      let randomIndex = Math.floor(Math.random() * array.length);
      if (_.indexOf(indexCache, randomIndex) === -1) {
        randomizedArray.push(array[randomIndex]);
        indexCache.push(randomIndex);
      }
    }
